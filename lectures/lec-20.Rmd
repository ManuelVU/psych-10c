...
we noticed that there was a pattern, were the model was underestimating 
...
- There is a lot of variability in our observations, but it looks like older
participants have faster response times in comparison with younger 
participants.

--

- Now we need include the predictor **age** to our model so that we can make 
...
and is responding to a figure that was rotated 11 degrees.
...
- We have just introduced new notation. When we want to denote the prediction
...
--
  ...
  - $(\beta_0)$: The estimated value of the intercept was `r round(betas[1],2)`, 
  which represents the expected response time when the other variables in the 
  model take the value of 0.
  
--

  - $(\beta_1)$: The estimated value of the slope associated with angle of 
  rotation was equal to `r round(betas[2],2)`, this means that the average 
  response time increases by approximately `r round(betas[2],0)` milliseconds 
  for each additional degree by which a figure is rotated.
...
--
 ...
 response time decreases by approximately `r abs(round(betas[3],0))` 
 milliseconds on average for each year.
...
and compare our new model with the Null and the simple linear regression.

---

## Adding predictions and errors

- The code that we need to use to add our predictions and errors to 
...
- Now we have all the values that we need to compare our 3 models:

--

  - Null: assumes that the response time of participants is constant regardless 
  of angle of rotation of the figure or participant's age.
...
- We will summarize the results using a table:
...
- In summary, the results from the model comparisons indicate that  both the 
angle of rotation of a figure and the age of the participants are good
predictors of the response time of participants in a mental rotation task.
...
to have lower response times on average for figures whit the same angle of 
rotation.

--

- Now we can evaluate if the multiple linear regression has the properties that 
...
- There is no pattern in the errors when graphed against the observation number.
...
distribution centered at 0 and with a variance of $\hat{\sigma}^2_m$ (MSE), and that 
there are no patterns when we graph the errors against other variables in the 
study, we can be more "confident" with our choice of a linear model.
...
--
  ...
  - The model that better accounts for our observation was a multiple linear
  regression that includes angle of rotation and participant's age as predictors. 
  The results suggest that the average response time of participants in a mental
  rotation task increases with the angle of rotation and decreases with 
  participant's age. The model can account for approximately 95% of the total
