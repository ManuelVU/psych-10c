...
- Last week we talk about factorial designs. A factorial design is a study where 
we have a single dependent variable (observation) denoted as $y_{ijk}$.

--

- In factorial designs we also have more than one independent variable which can 
...
$(j=1)$ of if the student belongs to the 2020 cohort $(j=2)$, and $k$ indicates
...
- This are two different models and have to be evaluated independently.
...
$(j=2)$, regardless of whether they took a statistics class or not.
...
a general value or response on the population (the **grand mean**) and the 
...
The last value $J$ is derived as the negative of the sum of the remaining 
effects.
...
anxiety levels of students, our first main effects model would be the 
