...
- And $\hat{\sigma}^2_0$ which is the average error or variability of our 
observations when we use $\hat{\mu}$ as a prediction of each observation.
...
- Finally, we are also interested in the Sum of Squared Errors of the Effects 
...
- The firs thing to do is add each models' predictions for each observation to 
our data.
...
# get predictions for the null model 
null_pred <- smokers %>% 
  summarise("pred" = mean(lung_capacity)) %>% 
  pull(pred)
...
smokers <- smokers %>% 
  mutate("pred_null" = rep(x = null_pred, times = n_total),
         "pred_eff" = ifelse(test = smoke_status == "non_smoker",
                             yes = eff_pred$prediction[1], 
                             no = eff_pred$prediction[2]))
```
...
- Now we can add the squared errors, we take the difference between each 
observation and a models' prediction and square it:
...
- Now our data file has the relevant variables:
...
for $\mu_1$ to be equal to $\mu_2$, which would be the same as saying that each
group has the same expected value...
...
- What does it mean for our estimators that the Null model is nested on the 
Effects model?

--

- Well this means that, the error or variability of the Null model will always 
be greater than or equal to the error of the Effects model.

--

- In other words, the worse that the Effects model can do is to be equal to the 
Null model, anything else will result in a lower error for the Effects model.
...
- Given that the $SSE_e$ (Sum of Squared Errors for the Effects model) is 
always **less than or equal** to the $SSE_0$ we can express the error accounted 
for by the Effects model as:
$$SSE_a = SSE_0 - SSE_e$$
...
- The total error is the error of the Null model $(SSE_0)$, and the error 
accounted for is $SSE_a$. Therefore, we can express the proportion of accounted 
error $(R^2)$ as:
...
(which can't happen unless our observations have no variability).
...
- $R^2$ will give us a first look at how well the Effects model is doing in any 
given situation.
...
always be less than or equal to the error of the the Null model.
...
- So we can interpret this number as "how bad does the model fit the data"

---

## Model complexity

- Model complexity is measured as a function of the number of parameters in a 
model that are associated with its predictions.
...
- Therefore, the BIC of the Null model is equal to: 
$$BIC_0 = n\ ln \left(\hat{\sigma}_0^2\right)\ +\ ln\left(n \right)$$
...
$$BIC_e = 8\ ln \left(5.79\right)\ +\ 2\ ln\left(8 \right) = 18.20$$
