...
distributed, however, we will compare our models by using the independent 0, 1
responses. 
...
|--------------------|:-----------:|:--------------:|:-------------:|
| Not vaccinated     | 40          | `r cov_prop$positive_test[1]` | `r round(cov_prop$positive_test[1]/40,2)` |
| Vaccinated         | 40          | `r cov_prop$positive_test[2]` | `r round(cov_prop$positive_test[2]/40,2)` |
...
$$\theta  = \frac{e^{\beta_0}}{1+ e^{\beta_0}}$$

--

- Thus, according to the Null model, the probability that any participant 
will test positive on a test is equal to $\frac{e^{\beta_0}}{1+ e^{\beta_0}}$
...
--
  ...
  positive for COVID regardless of vaccination status (because the Null 
...
- One disadvantage that we face now is that we no longer have a simple way 
...
- The question that we should be interested in is:
...
computer to process, we can work with the natural logarithm instead. The 
advantage here will be that the natural logarithm transforms multiplications 
into sums!
...
- For the binomial model, the value of the sum of the log-likelihoods $\sum_{i=1}^{N} ll(y_i)$ will always be between $-\infty$ and $0$.

--

- The closet it is to $0$ the better the model is (more negative values in a 
GLM indicate worse model predictions).

--

- When we multiply that value by a negative number that means that the larger 
the value of *fit* the worse the model will be, and so we should prefer models 
that have smaller BIC values.
...
# Complexity, the number of parameters of the Null is equal to 1
complexity <- 1 * log(n_total)

# Model fit, we can add all the values of the log-likelihood
fit <- -2 * sum(covid$loglikelihood_null)

# The BIC is the sum of model fit and model complexity
