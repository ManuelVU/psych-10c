...
a geometric figure (left), and then, after a a fixed period of time has elapsed 
...
if the second image was the same figure. 

--

- The age of participants in the experiment ranged from 7 to 21 years.

--

- The data from the looks like this:
...
that the focus of our research question is on the effect of the 
**angle of rotation** on response time, not the **age**.

--

- Because we are interested in the effect of angle of rotation we will start 
with a simple linear regression using only that predictor.

---

## Null model

- The first model is the Null, which assumes that there is no effect of angle 
of rotation on the time it takes participants to identify if the figure is the 
same but has been rotated (response time).

--

- Remember the model is formalized as:
...
- The second model is the simple linear regression. This model assumes that the
expected response time changes as a linear function of the degrees by which the 
...
- $\beta_0$ represents the response time to a figure that has been rotated by 0
degrees.
...
- $\beta_1$ represents the change in response time for a 1 degree increase 
in the rotation of the figure. For example, the change in response time to 
...
we expect response times to be one value for figures rotated 10 degrees
and a different one when the figure is rotated 10.1 degrees.

--

- In other words, our predictions about the expectation of response times
...
- The prediction of the linear model for a given angle of rotation $x_i$ is:
...
of the time it takes participants to identify if a figure is the same 
as the one they saw at the start of the trial that has been rotated.
...
--
  ...
  that it takes participants `r round(betas[1],0)` milliseconds on 
...
"**on average**", this is because the model's predictions are about the expected
...
or "**for each additional degree of rotation**". This is because each estimate
...
- As we have said before, if we add the difference between observation and 
...
- In general we want those differences to have two properties:
...
--
  ...
  2. There are no patterns, in the difference between observation and prediction 
...
represents the **angle of rotation** associated with the *i-th* observation and
$x_{i2}$ represents the **age** of the participant associated with the *i-th*
observation.
...
object is rotated by 0 degrees $(x_{i1} = 0)$ and the participant is 0 years 
old $(x_{i2} = 0)$.

--

- Typically, if the interpretation of the parameter doesn't make much sense in 
the context of a experiment (like a participant that is 0 years old), we just 
report the estimated value of the parameter. For example, 
"the estimate of the intercept was equal to..."
...
- The main difference from our previous interpretation, is that this time we 
have to interpret it as the change in our dependent variable when the other 
independent variable, in this case **age**, are held constant.

--

- If we allow both variables to change values at the same time, then we 
would not be able to interpret parameters as the change in our dependent 
variable, because the total change would be equal to the sum of both 
$\beta_1$ and $\beta_2$.

--

- Because of this, we always include the phrase "holding other variables 
constant" to our interpretation. This means that only the independent variable
associated to the $\beta$ value that we are looking at is allowed to change.
...
regression, the estimated values of the parameters that we had before will change.
...
to the second independent variable $(x_{i12})$.

--

- In other words, part of the change in a participants response time depends on 
the angle of rotation and some of that change depends on the age of the 
participant.
