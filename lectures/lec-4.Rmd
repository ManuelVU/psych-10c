...
- One of our main objectives in research is to contrast our beliefs about the
world with the outcomes of experiments.

--

- We do so by starting with some "verbal" statement or beliefs about the world 
...
with respect to our original beliefs or statements about the world.
...
- They are a way in which we can formalize our beliefs about probabilistic 
events.
...
- We can formalize these two beliefs using a statistical model.
...
- We moved from two verbal statements about our beliefs regarding the coin to
...
- As we can see in the plot, as the standard deviation $\sigma$ 
increases from 1 to 3, the normal distribution becomes more wide and less tall.

--

- This indicates that $\sigma^2$ and its root square $\sigma$ control the 
...
(or $\sigma$ in R), we have defined a Normal distribution completely. 

--

- In other words, we know the density (height of the curve) assigned to each 
value of the random variable.

--

- Another important thing to note is that both parameters are independent, in 
the sense that they control different aspects of the function. For example, 
we can have 2 Normal distributions that have the same variance and centered at 
different points.
...
same $\mu$ but different $\simga^2$'s.

---

## Statistical models

- Now that we have a notation for our observations $y_{ij}$.

--

- And we have a statistical model in the Normal distribution.
...
- Remember that our problem was that we wanted to know if people that smoke have 
lower lung capacity in comparison with people that do not smoke.
...
number on group $j = 1,2$ where $1$ represents the non-smokers.
...
- Now we can think of two possibilities, the first one is that there are 
**no differences** between non-smokers and smokers. In other words, that even
when our observations have some variability, lung capacity is not a function 
of smoking status.

--

- The second one (and the one that we might be more interested in) is that lung
capacity is a function of smoking status, in other words, that the 
groups are different.

--

- The first model is known as the **NULL Model**. A model that states that there 
are no difference in lung capacity between groups!

--

- We denote this model in the following way: $$y_{ij} \sim \text{Normal}(\mu,\sigma^2)$$
...
observation number $i$, and the group $j$ (smoking status), participant are all described by the same parameters.

--

- In other words, even though there might be some variability in our measures 
of lung capacity, non-smokers and smokers are all observations from the same 
distribution!

--

- A Normal distribution that is centered at some value $\mu$ and that has some
variability $\sigma^2$.

--

- Notice that we don't know the values of $\mu$ and $\sigma^2$ which define our
statistical model, however, we can graph what the Null Model expects the data to 
look like.
...
suitable values for the parameters $\mu$ and $\sigma^2$ which define our 
statistical model.
