...
participants regardless of treatment. $$y_{ij}\sim\text{Normal}(\mu,\sigma_0^2)$$
...
- We evaluate this model as the others, by calculating its predictions and error.
...
- From our results we can see that the best model was the Effects mode, 
...
- Participants that used the new drug for a week had an average blood pressure 
...
- Our estimators for the expected value of each group distribution (the average 
...
- To know add error bars to our estimators (AKA confidence intervals) we have
...
prediction for group $j$ and SSE is the sum of squared error of the model.
...
$n_j = 25$ and we have 3 predictions so $k=3$.
...
condition. Therefore, we need to calculate the sample size according to the 
model we selected.
