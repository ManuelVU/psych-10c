...
- We formalized a model that assumed that there are no differences on the 
anxiety levels of First year students between the 3 cohorts (Null model) that is 
...
2. Students in the 2019 cohort are different from students in the 2018 and 2020 
cohorts.
...
- First the estimator $\hat{\mu}_1$ will be the average anxiety level in the 
2018 cohort, while the estimator for the other 2 cohorts $\hat{\mu}$ will be the 
...
`r round(pred_2018$pred[1],2)` and for students in the 2019 and 2020 cohorts it 
was `r round(pred_2018$pred[2],2)`.
...
model. Again, we start by calculating the error for each observation by taking 
the difference between the observation and the model prediction ( $\hat{\mu}_1$ 
and $\hat{\mu}$, respectively) and then squaring that difference.
...
- The approach that we have taken here is a "brute force" approach, as we looked
at all the combinations between the cohorts to find the best model. We don't 
have to do this every time we have multiple groups.

--

- Next class we will see that, if we have a hypothesis about how our 
observations will behave, then we only need to look at the Null, Effects and
a specific model in order to answer our scientific question.
