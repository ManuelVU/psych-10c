...
```{r exmas-wide}
exams <- exams %>% 
  pivot_wider(names_from = exam, values_from = scores)
```
...
5. Using the new variables that you created calculate the SSE of each model and 
the mean SSE ($\sigma^2$) for each.
